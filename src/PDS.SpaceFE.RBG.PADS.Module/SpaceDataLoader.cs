...
namespace PDS.SpaceFE.RBG.PADS.Module
{
    /// <summary>
    ...
    public class SpaceDataLoader : BaseE4aMessageProcessor<SpaceE4A>
    {
        ...
        protected override void Process(SpaceE4A e4aDocument, E4aProcessContext context)
        {
            ...
            using (var split = context.Stopwatch.Start("Lot new Aggregates"))
            {
                operLotPADS = CreateOperLotAggregates(e4aDocument, context.QueueMessage);
            }
            //Calculating Wafer aggregates
            using (var split = context.Stopwatch.Start("Wafer new Aggregates"))
            {
        ...
        public SpacePads CreateOperWaferAggregates(SpaceE4A e4aEntry, SpacePads operLotPADS, IQueueMessage message)
        {
            ...
            {
                if (rawValues.MotherLotWafer?.Length > 8)
                {
                    if (wafers.Contains(rawValues.MotherLotWafer))
                        continue;

                    wafers.Add(rawValues.MotherLotWafer);
                }
            }
            foreach (string wafer in wafers)
            {
                string waf = wafer.Length == 11 ? wafer.Substring(wafer.Length - 2) : (wafer.Length == 10 ? wafer.Substring(wafer.Length - 1) : "");
                string motherLot = wafer[..8];
                string motherlotWafer = motherLot + ":" + waf;
                padsOperDoc = CreateOperWaferAggregate(e4aEntry, operLotPADS, motherlotWafer, message);
            }
            return padsOperDoc;
        }
        private SpacePads CreateOperWaferAggregate(SpaceE4A e4aEntry, SpacePads operLotPADS, string wafer, IQueueMessage message)
        {
            ...
            string waf = wafer.Length == 11 ? wafer.Substring(wafer.Length - 2) : (wafer.Length == 10 ? wafer.Substring(wafer.Length - 1) : "");
            string idOperWafAgg = GetOperWaferAggregationId(e4aEntry, wafer);

            var checkValueList = _padsDao.FindExistingWafDoc(operLotPADS.DataFlatMetaData.MeasLot.Substring(0,8),wafer, operLotPADS.ProductionAction.Id);
        ...
        private static string GetOperWaferAggregationId(SpaceE4A e4aEntry, string waferName)
        {
            ...
            return $"MotherlotWafer:{waferName}:SPACEAGGED1:RBG:FE:{facility}:{oper}:{subOper}:ProcessControl:1.0";
        }
    }
}
