...
namespace PDS.SpaceBE.WUX.Source.Module.Data
{
    ...
    public class SpaceDao
    {
        ...
        public IEnumerable<SpaceEntry> GetSpaceDatabaseEntries(DateTime startTime, DateTime endTime, long ldsId)
        {
            using (var conn = _databaseManager.OpenConnection(_connectionString))
            {
                var spaceDictionary = new Dictionary<string, SpaceEntry>();
                var parameters = new { StartTime = startTime, EndTime = endTime, LdsId = ldsId };
