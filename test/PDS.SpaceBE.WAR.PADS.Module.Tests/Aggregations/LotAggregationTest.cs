...
using PDS.SpaceBE.WAR.PADS.Module.Aggregations;
using PDS.SpaceBE.WAR.PADS.Module.Data.PADSModel;
using System.Collections.Generic;
using Moq;

namespace PDS.SpaceBE.WAR.PADS.Module.Tests.Aggregations
{
    /// <summary>
    ...
    public class LotAggregationTest
    {
        ...
        public void TestCreateNew()
        {
            ...
            Assert.AreEqual("Measlot:3A140739:SPACEAGGED3:WAR:BE:HD:Paste Printer::ProcessControl:1.0", newLotPads.SearchPatterns.SpaceKey);
        }
        ...
        {
            var spaceE4A = JsonUtils.FromJson<SpaceE4A>(Properties.LotResources.TestUpdateNewParam);
            var checkDoc = JsonUtils.FromJson<SpacePads>(Properties.LotResources.PadsDoc);
            var aggregation = new LotAggregation(spaceE4A, new Mock<IQueueMessage>().Object);
            var newLotPads = aggregation.UpdateExisting(checkDoc);
            Assert.AreEqual("Measlot:3A140739:SPACEAGGED3:WAR:BE:HD:Paste Printer::ProcessControl:1.0", newLotPads.SearchPatterns.SpaceKey);
